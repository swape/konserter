	...
	function filterList(innerConcertList: ConcertObjectType[], filterValue: string) {
		if (filterValue) {
			return innerConcertList.filter((concert: ConcertObjectType) => {
				return (
					isIn(`${concert.rating}`, filterValue) ||
					isIn(concert.date, filterValue) ||
					isIn(concert.artist, filterValue) ||
					isIn(concert.venue, filterValue) ||
					isIn(concert.festival, filterValue)
				);
			});
		}
...
<div class="box">
	...
	<ul class="p-3">
		{#each thisYearList as concert}
			<li>
				<button on:click={() => gotoConcert(concert.id)} class="concert-button">
